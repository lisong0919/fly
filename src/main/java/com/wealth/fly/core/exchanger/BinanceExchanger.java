...
public class BinanceExchanger implements Exchanger {
    ...
    private String createOrder(Order order, String orderType, String stopPrice) throws IOException {
        ...
        if (code == -2019) {
            throw new InsufficientBalanceException(JsonUtil.getString("/msg", jsonNode), -2019);
        }
    ...
    private String signAndPostRequest(String url, LinkedHashMap<String, Object> request, String desc)
            throws IOException {
    ...
    private String generateSignedRequest(String tobeSign) {
        String sign = new HmacUtils(HmacAlgorithms.HMAC_SHA_256, account.getSecretKey())
                .hmacHex(tobeSign.getBytes(StandardCharsets.UTF_8));
    ...
    public void cancelOrder(String instId, String orderId) throws IOException {
        String url = HOST + "/dapi/v1/order";
        String params =
                "symbol=" + instId + "&orderId=" + orderId + "&timestamp=" + System.currentTimeMillis();
    ...
    @Override
    public List<InstrumentInfo> listInstrumentInfo(String instType, String instFamily)
            throws IOException {
        return null;
    }
    ...
    public List<FundingRate> listFundingRateHistory(String symbol, Date startTime, Date endTime, Integer limit) throws IOException {
        ...
        String response = signAndGetRequest(HOST + "/dapi/v1/fundingRate", params, "查历史资金费率");
        JsonNode jsonNode = JsonUtil.readValue(response);


        List<FundingRate> fundingRates = new ArrayList<>();
        Iterator<JsonNode> jsonNodeIt = jsonNode.elements();
        while (jsonNodeIt.hasNext()) {
            ...
            fundingRate.setMarkPrice(new BigDecimal(node.get("markPrice").asText()));
